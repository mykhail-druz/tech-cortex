...
export const BrandFilterComponent: React.FC<BrandFilterComponentProps> = ({
  filter,
  onChange,
}) => {
  ...
  const handleToggleOption = (brandValue: string) => {
    const newValue = filter.value.includes(brandValue)
      ? filter.value.filter(v => v !== brandValue)
      : [...filter.value, brandValue];
